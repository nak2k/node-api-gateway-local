...
function awsIntegration(options, req, res, result, callback) {
  const {
    resource,
    pathParameters,
    resourceMethod,
  ...
  sts.assumeRole({
    RoleArn: credentials,
    RoleSessionName: 'api-gateway-local',
  }, (err, data) => {
    if (err) {
      return callback(err);
    }
    ...
    const signedReqOptions = aws4.sign(reqOptions, {
  ...
  const [err, integrationParameters] = evalParameters(requestParameters, {
    method: {
      request: {
        ...
        header: makeHeadersFromRaw(req.rawHeaders),
        body: req.body,
      },
    },
    stageVariables: {
