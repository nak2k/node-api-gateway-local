  ...
  return (req, res, next) => {
    waterfall([
      ...
      (result, callback) => {
        ...
        const spawnOptions = {
          dir: resolve(configDir, lambda.dir),
          handler: lambda.handler,
          region,
          command: lambda.command,
          moduleDir: lambda.moduleDir && resolve(configDir, lambda.moduleDir),
          env: lambda.env,
          lambdaEnv: lambda.lambdaEnv,
          additionalNodePath: lambda.additionalNodePath &&
            lambda.additionalNodePath.split(':').map(p => resolve(configDir, p)).join(':'),
          babel: lambda.babel,
        };
